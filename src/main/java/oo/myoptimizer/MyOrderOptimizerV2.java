...
public class MyOrderOptimizerV2 implements OrderOptimiser {
    ...
    public void optimise(List<OrderInstruction> orderInstructions, OrderManager orderManager) {
        ...
        synchronized(orderManager)
        {
            ...
            final List<Block> blocks = new ArrayList<>(orderInstructions.size() + openOrders.size());
            ...
            for(Block block : blocks) {
                Dispatcher.dispatchWithCountDown(block, latch);
            }
            try {
                latch.await();
            } catch (InterruptedException e) {
                Util.log("CountDownLatch Interrupted");
            }
